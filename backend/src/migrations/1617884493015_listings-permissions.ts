...
export async function up(pgm: MigrationBuilder): Promise<void> {
  ...
  pgm.sql('GRANT UPDATE ON listings.users TO standard');
...
export async function down(pgm: MigrationBuilder): Promise<void> {
  ...
  pgm.sql('REVOKE UPDATE ON listings.users FROM standard');
  ...
  pgm.sql(
    'REVOKE USAGE, SELECT ON ALL SEQUENCES IN SCHEMA listings FROM admin'
  );
